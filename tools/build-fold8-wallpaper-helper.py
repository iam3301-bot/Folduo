#!/usr/bin/env python3
"""使用 Android SDK 和 Java 17 构建 Fold8 双屏壁纸辅助工具，不操作设备。"""
import argparse
import os
...
def main():
    ...
    output = tools / 'fold8-wallpaper-setup.jar'
    with tempfile.TemporaryDirectory(prefix='wallpaper-', dir=build) as temporary:
        ...
        subprocess.run([javac, '--release', '17', '-encoding', 'UTF-8', '-cp', str(android),
                        '-d', str(classes), str(tools / 'Fold8WallpaperSetup.java')], check=True)
        ...
        dex_jar = temp / 'fold8-wallpaper-setup.jar'
